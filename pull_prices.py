...
def is_local(origin):
    local_indicators = ['500 miles', 'NY', 'New York', 'VT', 'Vermont', 'MA', 'Massachusetts', 
...
def scrape_produce_prices():
    ...
    if len(tables) > 0:
        ...
        print("No tables found on the page. The website structure might have changed.")


def generate_html(df):
    # Split the data into local and non-local
    local_df = df[df['is_local'] == True].sort_values('price')
    non_local_df = df[df['is_local'] == False].sort_values('price')
